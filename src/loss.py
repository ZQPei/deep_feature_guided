...
class GANLoss(nn.Module):
    def __init__(self, gan_mode='nsgan', target_real_label=1.0, target_fake_label=0.0,
    ...
    def get_target_tensor(self, input, target_is_real):
        if target_is_real:
            if self.real_label_tensor is None:
                self.real_label_tensor = self.Tensor(1).fill_(self.real_label).to(input)
                self.real_label_tensor.requires_grad_(False)
            return self.real_label_tensor.expand_as(input)
        else:
            if self.fake_label_tensor is None:
                self.fake_label_tensor = self.Tensor(1).fill_(self.fake_label).to(input)
    ...
    def loss(self, input, target_is_real, for_discriminator=True):
        if self.gan_mode == 'nsgan':  # cross entropy loss
...
class VGG19(torch.nn.Module):
    def __init__(self, normalize=True):
        ...
        pretrained_path = "./pretrained/vgg19_places2_ep83_acc87.pth"
    ...
    def forward(self, x):
        if self.normalize:
            x = x.sub(self.mean[None, :, None, None].to(x)).div_(self.std[None, :, None, None].to(x))
...
class FeatureLoss(torch.nn.Module):
    ...
    def forward(self, fmap1, fmap2):
        return self.criterion(fmap1, fmap2)